...
def main():
    ...
    for iter in iters:
        ...
        for epsilon in epsilons:
            datasets.append(torch.load(f'./adversarial_example/snn_label/{iter}_{name}_adversarial_{epsilon}.pth'))
            ...
            with torch.no_grad():
                for i, (inputs, labels) in enumerate(testloader):
                    ...
                    for j in range(inputs.size(0)):
                        ...
                        save_image_with_heatmap(inputs[j], sam, times, image_index, labels[j].item(), predicted_snn[j].item(), folder=os.path.join(folder_name, f'{iter}/{epsilon}'))
