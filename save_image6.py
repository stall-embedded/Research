...
def main():
    ...
    model_cnn.eval()
    for iter in iters:
        folder_name = f"./result/image/grad_cam/cnn/{name}/{iter}/"
        datasets = []
        ...
        for dataset, epsilon in zip(datasets, epsilons):
            ...
            for i, (inputs, labels) in enumerate(testloader):
                ...
                for j in range(inputs.size(0)):
                    save_image_with_heatmap(inputs[j], g_cam, i, labels[j].item(), predicted_cnn[j].item(), os.path.join(folder_name, f"{epsilon}"))
