...
class BasicModel(nn.Module):
    def __init__(
        ...
        self.features = nn.Sequential(
            # preparation
            layer.Conv2d(num_channels, c[0], kernel_size=3, stride=1, padding=1, bias=False),
            neuron.LIFNode(tau = TAU, surrogate_function=SURROGATE),
            # block 1
            layer.Conv2d(c[0], c[1], kernel_size=3, stride=1, padding=1, bias=False),
            neuron.LIFNode(tau = TAU, surrogate_function=SURROGATE),
            layer.AvgPool2d(2),
        ...
        self.features[9].register_forward_hook(self.forward_hook)
        self.features[9].register_full_backward_hook(self.backward_hook)
