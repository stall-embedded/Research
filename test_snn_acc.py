...
def main():
    ...
    for epsilon in epsilons:
        testsets.append(torch.load(f'./adversarial_example/snn_label/mini_{name}_adversarial_{epsilon}.pth'))
    ...
    with torch.no_grad():
        for epsilon, testset in zip(epsilons, testsets):
            ...
            conf_matrix_df.to_csv(f'mini_{name}_confusion_matrix_{epsilon}.csv', index=False)

        results_df = pd.concat(results_df_list, ignore_index=True)
        results_df.to_csv(f'model_mini_{name}_performance.csv', index=False)
